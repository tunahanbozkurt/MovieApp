...
fun SignUpScreen(
    ...
    Column(
        ...
        CommonTextField(
            ...
            labelText = "Password",
            visualTransformation = if (passwordField.isVisible) VisualTransformation.None else PasswordVisualTransformation(),
            modifier = Modifier.fillMaxWidth(),
            isError = passwordField.hasError,
            trailingIcon = {
                Icon(
                    imageVector = if(passwordField.isVisible) ImageVector.vectorResource(id = R.drawable.baseline_visibility_off_24) else ImageVector.vectorResource(
                        id = R.drawable.baseline_visibility_24
                    ),
                    contentDescription = null,
                    tint = MaterialTheme.localColor.textGrey,
                    modifier = Modifier.clickable { viewModel.handleUIEvent(SignUpScreenUIEvent.ShowPassword) }
                )
            },
            onValueChange = { viewModel.handleUIEvent(SignUpScreenUIEvent.EnteredPassword(it)) }
        )
        ...
        Row(
            ...
            Text(text = annotatedString, style = MaterialTheme.localFont.mediumBody, color = MaterialTheme.localColor.textGrey)
        }
